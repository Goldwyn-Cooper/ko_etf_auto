...
KIS_ACCOUNT_BALACNE_COL = ("매입금액", "평가금액", "평가손익금액", "신용대출금액", "실제순자산금액", "전체비중율")
...
def get_token_from_server() -> tuple:
    payload = dict(grant_type="client_credentials", appkey=appkey, appsecret=appsecret)
    json = requests.post(KIS_DOMAIN + "/oauth2/tokenP", json=payload).json()
    return (json.get("access_token"), json.get("access_token_token_expired"))


def save_token(token: str, expired: str) -> tuple:
    payload = dict(cano={"S": cano}, token={"S": token}, expired={"S": expired})
...
def get_params():
    return dict(CANO=cano, ACNT_PRDT_CD="01", INQR_DVSN_1="", BSPR_BF_DT_APLY_YN="")


def get_account_balance(token):
    PATH = KIS_TRADING + "/inquire-account-balance"
    json = requests.get(
        KIS_DOMAIN + PATH, params=get_params(), headers=get_headers(token, "CTRP6548R")
    ).json()
...
def get_balance(token):
    ...
    json = requests.get(
        KIS_DOMAIN + PATH, params=params, headers=get_headers(token, "TTTC8434R")
    ).json()
    df = (
        pd.DataFrame(json.get("output1"), columns=KIS_BALANCE_COL_RAW)
        .astype({"hldg_qty": int, "evlu_pfls_rt": float})
        .query("hldg_qty > 0")
    )
...
def sell_market_order(symbol, qty, token):
    ...
    json = requests.post(
        KIS_DOMAIN + KIS_TRADING + "/order-cash",
        json=trading_payload(symbol, qty),
        headers=get_headers(token, "TTTC0801U"),
    ).json()
...
def buy_market_order(symbol, amt, price, token):
    ...
    json = requests.post(
        ...
        headers=get_headers(token, "TTTC0802U"),
    ).json()
    msg_cd, msg = json.get("msg_cd"), json.get("msg1")
    send_message((f"🚀 {msg}" if msg_cd == "APBK0013" else f"😱 {msg_cd} : {msg}"))
...
def enter_position(
    balance: pd.DataFrame, candidate: pd.DataFrame, budget: int, token: str
):
    enter_table = (
